...
!----------------------------------------------------------------------
!
! Solve schrodinger equation of hydrogen-like atom/ion via expansion on a 
! Gaussian basis set
! Requires: lapack blas
! Originate from Paolo Giannozzi' lecture notes "Numerical Methods in
! Quantum Mechanics"
! Modified by zito, 2018/5/23
!
!----------------------------------------------------------------------
      implicit none
      integer, parameter :: dp = selected_real_kind(14,200)
      real (dp), parameter :: e2=2.0_dp, pi=3.14159265358979_dp, & 
                              tpi=2.0_dp*pi
...
! read from specified file
      ...
      do i=1,n_alpha
        if ( iunit == 5 ) then
          ...
          read (iunit,*) alpha(i)
          write (*,'(" Gaussian # ",i3," coefficient >> ",f10.6)') i, &
                 alpha(i)
        end if
...
! Allocate arrays
!
      allocate (s(n_alpha, n_alpha), h(n_alpha,n_alpha),    &
                v(n_alpha,n_alpha), e(n_alpha) )
!
!-----------------------------------------------------------------------
!                   s-type basis set
! Assign values of overlap integrals and of matrix elements of the 
! hamiltonian on the S basis:
!
!-----------------------------------------------------------------------
      do ia=1,n_alpha
        do ib=1,n_alpha
          ...
          h(ia,ib) = s(ia,ib)*6.0_dp*alpha(ia)*alpha(ib)/aa - & 
                     e2*zeta*tpi/aa 
        end do
      end do
  !
  ...
  !
  !
  !       Write to output file the coefficients of the lowest-energy
  !       state:
  !
...
!
  ...
  integer, intent(in) :: n, &! dimension of the matrix to be
diagonalized
                         ldh ! leading dimension of h,s and v, as
declared
  ...
  !
  !       Copy S into an auxiliary matrix because dsyev destroys the
  !       matrix
  !
  aux = s
  !
  ...
  !
  !   Keep only linearly independent combinations (within a given
  !   threshold)
  !   store into matrix "aux" the eigenvectors of S divided by the
  !   squares
  !   of the eigenvectors
  !
  nn = 0
  ...
  !
  call dgemm ( 'N', 'N', n, nn, n, 1.0_dp, h, ldh, aux, ldh, 0.0_dp, v,
ldh )
  !
  ...
  !
  allocate (h1(nn,nn) )
  call dgemm ( 'T', 'N', nn, nn, n, 1.0_dp, aux, ldh, v, ldh, 0.0_dp,
h1, nn )
  !
  ...
  !
  call dgemm ( 'N', 'N', n, nn, nn, 1.0_dp, aux, ldh, h1, nn, 0.0_dp, v,
ldh )
  !
