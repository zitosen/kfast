...
#
#
  echo "Extracting l matrix"
  dof=`grep 'Deg. of freedom' $FILE | sed 's/[^0-9]//g'`
  l_row=$(( dof + 6 + 7 ))
#  ibegin=`grep -n 'Frequencies ---' $FILE | sed -n '1p' | awk '{print $1}' | sed 's/[^0-9]//'`
#  ibegin=`expr $ibegin + 8`
#  iend=$(( ibegin + dof + 6 - 1 ))
#  sed -n "$ibegin,$iend p" $FILE 
  grep -A $l_row 'Frequencies ---' $FILE | awk '$1!="--"{print $0}' > lmat.dat 
